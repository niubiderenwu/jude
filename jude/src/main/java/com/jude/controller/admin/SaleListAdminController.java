...
public class SaleListAdminController {
	 ...
	 * @throws Exception
	 */
	@RequestMapping("/listSaleGoods")
	@RequiresPermissions(value = { "商品销售统计" })
	public Map<String,Object> listSaleGoods(Date bSaleDate,Date eSaleDate,String code,String name,Integer typeId)throws Exception{
		Map<String, Object> resultMap = new HashMap<>();

		List<SaleListVo> saleListVoList =  saleListService.queryGoodsListInfo(bSaleDate,eSaleDate,code,name,typeId);
		resultMap.put("rows", saleListVoList);
		return resultMap;
	}
	...
	public Map<String,Object> save(SaleList saleList,String goodsJson)throws Exception{
		...
		//判断销售出库不能大于库存数量
		for (SaleListGoods saleListGoods2 : plgList) {
			String code = saleListGoods2.getCode();
			//根据商品code，查询商品
			Goods goods = null;
			goods = goodsService.getCode(code);
			if(saleListGoods2.getNum()>goods.getInventoryQuantity()){
				resultMap.put("false", "库存不够");	
				return resultMap;
			}
		}
